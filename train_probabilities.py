...
from model_probabilities import RepresentationUNet
...
def args_parser():
    ...
    parser.add_argument("--superpixels_no", type=int, default=1000, help="number of superpixels")
...
class SuperpixelLoss(nn.Module):
    def __init__(self, coefficients=(1, 1, 1), eps=1e-8):
    ...
    def forward(self, features, probabilities):
        ...
        _, p, _, _ = probabilities.shape

        # Uniformity of volumes
        volumes = probabilities.sum(dim=(2, 3))  # [b, p]
        volume_loss = volumes.var(dim=1, correction=0).mean()

        # Interclass variance
        features_flat = features.view(b, f, -1)  # [b, f, hw]
        probs_flat = probabilities.view(b, p, -1)  # [b, p, hw]
        vals = torch.einsum('bfc,bpc->bfp', features_flat, probs_flat)  # [b, f, p]
        means = vals / (volumes.unsqueeze(1) + self.eps)  # [b, f, p]
        interclass_loss = -means.var(dim=2, correction=0).mean()  # variance over superpixels, mean over b and f

        # Intraclass variance
        squared_features = features * features
        squared_flat = squared_features.view(b, f, -1)
        squared_vals = torch.einsum('bfc,bpc->bfp', squared_flat, probs_flat)  # [b, f, p]
        squares = squared_vals / (volumes.unsqueeze(1) + self.eps)  # [b, f, p]
        intraclass_loss = (squares - means * means).sum(dim=2).mean()  # sum over superpixels, mean over b and f

        superpixel_loss = (self.coefficients[0] * volume_loss +
                           self.coefficients[1] * interclass_loss +
                           self.coefficients[2] * intraclass_loss)

        return {
            "volume": volume_loss,
            "interclass": interclass_loss,
            "intraclass": intraclass_loss,
            "superpixel_total": superpixel_loss
        }
...
class RepresentationLoss(nn.Module):
    ...
    def forward(self, pred_reps, target_reps):
        ...
        representation_loss = (1 - self.contrastive_rate) * similarity + self.contrastive_rate * contrastiveness
        ...
        return {
            ...
            "representation_total": representation_loss,
        }
...
def train(args):
    ...
    model_name = (
        f"prob_bs{args.batch_size}_lr{args.lr}_c{args.classes}_cr{args.contrastive_rate}"
        f"_ps{'_'.join([str(p) for p in args.patch_size])}_spn{args.superpixels_no}"
    )
    ...
    loss_probabilities = SuperpixelLoss()
    loss_representation = RepresentationLoss(contrastive_rate=args.contrastive_rate)
    ...
    model = RepresentationUNet(
        ...
        num_of_superpixels=args.superpixels_no,
    ...
    try:
        for epoch in range(loaded_epoch_no + 1, loaded_epoch_no + 1 + args.epochs):
            ...
            for images in tqdm(data_loader_train, total=len(data_loader_train)):
                ...
                optimizer.zero_grad()
                representations, features, probabilities = model(images, patches_coords)
                loss_rep = loss_representation(representations, target)
                loss_prob = loss_probabilities(features, probabilities)
                tot_loss = loss_rep["representation_total"] + loss_prob["superpixel_total"]
                tot_loss.backward()
                optimizer.step()
                for k, v in loss_rep.items():
                    if f"train_{k}" in epoch_losses:
                        epoch_losses[f"train_{k}"].append(v.item())
                    else:
                        epoch_losses[f"train_{k}"] = [v.item()]
                for k, v in loss_prob.items():
                    if f"train_{k}" in epoch_losses:
                        epoch_losses[f"train_{k}"].append(v.item())
                    else:
                        epoch_losses[f"train_{k}"] = [v.item()]
                if "train_loss_total" in epoch_losses:
                    epoch_losses["train_loss_total"].append(tot_loss.item())
                else:
                    epoch_losses["train_loss_total"] = [tot_loss.item()]
                ...
                for images in tqdm(data_loader_test, total=len(data_loader_test)):
                    ...
                    for k, v in loss_prob.items():
                        if f"test_{k}" in epoch_losses:
                            ...
                            epoch_losses[f"test_{k}"] = [v.item()]
                    if "test_loss_total" in epoch_losses:
                        epoch_losses["test_loss_total"].append(tot_loss.item())
                    else:
                        epoch_losses["test_loss_total"] = [tot_loss.item()]
