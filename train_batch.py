...
from evaluate_batch import rep_model
...
def train(args):
    ...
    writer = SummaryWriter(os.path.join("runs", f"cr={args.contrastive_rate}"))
    ...
    try:
        for epoch in range(loaded_epoch_no + 1, loaded_epoch_no + 1 + args.epochs):
            ...
            for images in tqdm(data_loader_train, total=len(data_loader_train)):
                ...
                optimizer.zero_grad()
                output = model(images, patches_coords, args.patch_size)
